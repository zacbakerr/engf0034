...
class RiverObject():
    ...
    def contains(self, frog):
        ...
        if frog_y != self.y or frog_x < self.x or frog_x > self.x + self.width:
            return False
        if isinstance(self, Turtle) and self.is_sunk():
            return False
        return True
...
class Model():
    ...
    def create_homes(self):
        ...
        for i in range(5):  # Changed from range(0,6) to range(5)
    ...
    def died(self):
        if not self.paused:  # Prevent multiple deaths while paused
            self.lives -= 1
    ...
    def check_pause(self):
        if self.paused and time.time() > self.pause_end_time:
            self.paused = False
            self.dont_update_speed = True
        ...
        if on_log is None:
            ...
            for log in self.logs:
                if log.contains(self.frog):
                    on_log = log  # Fixed typo: changed 'on_long' to 'on_log'
                    break
        if on_log is None:
    ...
    def check_frog_entering_home(self):
        ...
        (x, y) = self.frog.get_position()
        for i in range(5):  # Changed from range(0, 5) to range(5)
            if abs(self.homes_x[i] - x) < GRID_SIZE/2 and not self.homes_occupied[i]:
                #we're in a free home
                self.frog_is_home(i)
                return
        
        # If we've reached this point, the frog is not in a valid home
        self.died()
